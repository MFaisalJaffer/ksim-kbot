...
|---|---|---|
| **Constant torque limit (sim) vs velocity-dependent limit (real)** | Real BLDC motors lose motoring torque as speed rises (back-EMF). Braking torque (regenerative, opposite to ω) is unaffected. Sim with constant clip lets the policy demand peak torque at any joint speed — actions the real motor cannot execute. | `TVCurveMITActuators` applies **direction-aware clipping**: when `sign(ctrl)==sign(qvel)` (motoring), limit by interpolated T-V curve; when opposite (braking/regenerating), apply only the constant `ctrl_clip`. Matches real BLDC physics — full braking available at any speed. |
