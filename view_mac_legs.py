"""macOS-compatible policy viewer for the LEGS-ONLY kbot-v2-legs robot.

Usage:
    mjpython view_mac_legs.py --ckpt /path/to/ckpt.bin
...
No arm controls — this is the legs-only model (10 DOF, torso + legs only).
"""
...
def run_viewer_with_policy(ckpt_path: str) -> None:
    ...
    class KeyboardTask(KbotLegsWalkingRNNTask):
    ...
    with task, jax.disable_jit():
        ...
        print("\nLaunching legs-only viewer with keyboard control:")
...
def main():
    parser = argparse.ArgumentParser(description="macOS K-Bot legs-only policy viewer")
    parser.add_argument(
        "--ckpt",
